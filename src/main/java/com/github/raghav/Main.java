...
public class Main {
  ...
  private static Configuration setConf() {
    ...
    System.setProperty("java.security.krb5.realm", "CORPDEV.ORG.COM");
    System.setProperty("java.security.krb5.kdc", "localhost:88");
    conf.set("hadoop.security.authentication", "kerberos");
    conf.set("hadoop.security.authorization", "true");
    conf.set("dfs.namenode.kerberos.principal", "nn/_HOST@CORPDEV.ORG.COM");
    conf.set("dfs.namenode.keytab.file", "/etc/security/keytabs/hdfs.headless.keytab");
    conf.set("dfs.datanode.kerberos.principal", "dn/_HOST@CORPDEV.ORG.COM");
    conf.set("dfs.datanode.keytab.file", "/etc/security/keytabs/hdfs.headless.keytab");

    UserGroupInformation.setConfiguration(conf);
    try {
      UserGroupInformation.loginUserFromKeytab(
          "hdfs-optimus@CORPDEV.VISA.COM", "/etc/security/keytabs/hdfs.headless.keytab");
    } catch (Exception e) {
